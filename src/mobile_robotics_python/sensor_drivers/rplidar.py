...
class RPLidarImpl:
    ...
    def _read_response_descriptor(self):
        ...
        if len(raw_desc) < 6:
            raise IOError(
                "RPLIDAR timed out while waiting for command response"
            )  # noqa: E501

        if raw_desc[1] != 0x5A:
            raise IOError(
                "RPLIDAR communication error (response descriptor header not found)"
            )  # noqa: E501

        t = raw_desc[2] | (raw_desc[3] << 8) | (raw_desc[4] << 16) | (raw_desc[5] << 24)
    ...
    def poll_express_scan_samples(self):
        ...
        for i in range(packets_available):
            ...
            for cabin_idx in range(16):
                ...
                k = cabin_idx  # TODO THIS IS WRONG
    ...
    def get_sample_period(self):
        ...
        if len(data) < 4:
            raise IOError(
                "RPLIDAR timed out while waiting for sampling period"
            )  # noqa: E501
